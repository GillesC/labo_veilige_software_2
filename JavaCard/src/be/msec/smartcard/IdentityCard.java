...
public class IdentityCard extends Applet implements ExtendedLength {
	...
	public void getCertificate(APDU apdu){
		if(!pin.isValidated()) ISOException.throwIt(SW_PIN_VERIFICATION_REQUIRED);
		else{
			apdu.setOutgoing();
			...
			while(toSend > 0){
				short length;
				if(toSend > 240) length = (short) (toSend - 240);
				else length = toSend;
				apdu.sendBytesLong(certificate,(short)prevLength,(short) toSend);
				prevLength = toSend;
			}
			
			//apdu.setOutgoingLength((short) 240);
			//apdu.sendBytesLong(certificate,(short)240,(short)(certificate.length-240));
	}
	}
}
